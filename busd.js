...
async function main() {
    ...
    provider.on("block", async (blockNumber) => {
        ...
        if (!foundPairAddress) {
            pairContract = await FactoryContract.getPair(ContractAddress.BUSD, ContractAddress.TOKEN);
        ...
        if (!buySuccess && foundPairAddress && (foundLiquidity || !checkLiquidity)) {
            ...
            if (!buying) {
                const BUSDAmountToPay = ethers.utils.parseUnits(Config.BUSD_USE_AMT); // 0.350 BNB
                const estimateTokenReceive = await RouterContract.getAmountsOut(BUSDAmountToPay, [BUSD.address, TOKEN.address]);
                ...
                console.log(`Swapping ${ethers.utils.formatUnits(BUSDAmountToPay.toString())} BUSD for ${ethers.utils.formatUnits(estimateTokenReceive[1].toString())} ${tokenSymbol}`)
                ...
                const buyTrx = await RouterContract.swapExactTokensForTokens(
                    BUSDAmountToPay.toString(),
                    minTokenReceive.toString(),
                    [BUSD.address, TOKEN.address],
                    wallet.address,
                    deadline,
                    {
                        value: 0,
